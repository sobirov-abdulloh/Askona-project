...
function Register() {
  ...
  const sendOtpHandler = (e) => {
    ...
    axios
      .post("https://askona.herokuapp.com/api/v1/auth/", postData)
      .then((res) => {
  ...
  const handleSignUp = (e) => {
    ...
    axios
      .post("https://askona.herokuapp.com/api/v1/auth/", postAuthData)
      .then((res) => {
        if (res.data?.Error) {
          alert(res.data?.Error);
          return;
      ...
      >
        ...
        <Box sx={style} className="rounded-xl">
          <Box sx={{ width: "100%" }}>
            <Box sx={{}}>
              <button
                className="absolute top-[-10px] right-[-10px] text-center"
                onClick={handleClose}
              >
                {" "}
            ...
            <TabPanel value={value} index={0}>
              ...
              >
                ...
                />
                <button className="text-center bg-[#00b6c9] w-full text-white p-2 rounded">
                  Зарегистрироваться
                </button>
              </form>
            </TabPanel>
      ...
      >
        ...
        <Box sx={style} className="rounded-xl">
          <Box sx={{ width: "100%" }}>
            ...
            <form
              onSubmit={checkVerificationHandler}
              action=""
              className="flex flex-col items-center gap-5"
            >
              <input
                onChange={(e) => setOtp(e.target.value)}
                className="w-full border-[1px] outline-[#00B6C9] rounded border-gray-400 px-3 py-1"
                required
                type="number"
